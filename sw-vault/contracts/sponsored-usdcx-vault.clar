;; title: stacks wars sponsored usdcx vault
;; author: flames.stx
;; version: v1
;; summary: A sponsored USDCx (SIP-010) vault contract for Stacks Wars.
;; description: A sponsored USDCx vault contract for Stacks Wars where the
...
(define-public (join)
	(let
		...
		(if (is-eq sender DEPLOYER)
			(try! (contract-call? 'SP120SBRBQJ00MCWS7TM5R8WJNTTKD5K0HFRC2CNE.usdcx transfer POOL-SIZE sender (as-contract tx-sender) none))
			true
		)
...
(define-public (leave (signature (buff 65)))
	(let
		...
		(if (is-eq sender DEPLOYER)
			(try! (as-contract (contract-call? 'SP120SBRBQJ00MCWS7TM5R8WJNTTKD5K0HFRC2CNE.usdcx transfer POOL-SIZE tx-sender sender none)))
			true
		)
