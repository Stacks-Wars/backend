;; title: stacks wars usdcx vault
;; author: flames.stx
;; version: v1
;; summary: A USDCx (SIP-010) vault contract for Stacks Wars.
;; description: A USDCx vault contract for Stacks Wars, allowing players to join
;;				a game lobby by depositing an entry fee, leave the lobby to
...
(define-public (join)
	(let
		...
		(try! (contract-call? 'SP120SBRBQJ00MCWS7TM5R8WJNTTKD5K0HFRC2CNE.usdcx transfer ENTRY-FEE sender (as-contract tx-sender) none))
...
(define-public (leave (signature (buff 65)))
	(let
		...
		(try! (as-contract (contract-call? 'SP120SBRBQJ00MCWS7TM5R8WJNTTKD5K0HFRC2CNE.usdcx transfer ENTRY-FEE tx-sender sender none)))
...
(define-public (claim (amount uint) (signature (buff 65)))
	(let
		...
		(try! (as-contract (contract-call? 'SP120SBRBQJ00MCWS7TM5R8WJNTTKD5K0HFRC2CNE.usdcx transfer fee-amount tx-sender FEE-WALLET none)))

		;; Transfer reward to player
		(try! (as-contract (contract-call? 'SP120SBRBQJ00MCWS7TM5R8WJNTTKD5K0HFRC2CNE.usdcx transfer reward-amount tx-sender sender none)))
...
(define-public (kick (player principal) (signature (buff 65)))
	(let
		...
		;; Transfer entry fee from contract back to player
		(try! (as-contract (contract-call? 'SP120SBRBQJ00MCWS7TM5R8WJNTTKD5K0HFRC2CNE.usdcx transfer ENTRY-FEE tx-sender player none)))
