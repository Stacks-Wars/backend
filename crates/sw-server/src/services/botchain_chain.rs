//! BOT Chain USDT balance + activity for custodial wallets.
...
pub async fn get_balance(state: &AppState, user_id: UserId) -> AppResult<WalletBalance> {
    ...
    .await
    {
        Ok(amount) => amount,
        Err(err) => {
            tracing::error!(error = %err, "botchain USDT balance read failed");
    ...
    Ok(WalletBalance {
        ...
        chain: wallet.chain.parse().unwrap_or(ChainId::Botchain),
        available_micro,
        updated_at: Utc::now(),
        cached: false,
    })
...
pub async fn list_activity(
    ...
    if undeployed(&state.config.botchain_usdt) {
        return Ok(Vec::new());
    }
    ...
    match fetch_activity(
        &state.config.botchain_rpc_url,
        &state.config.botchain_usdt,
        &state.config.botchain_vault,
        &state.config.botchain_platform_wallet,
        &wallet.address,
        cap,
    )
    .await
    {
        Ok(items) => Ok(items),
        Err(err) => {
            tracing::warn!(error = %err, "botchain activity read failed");
            Ok(Vec::new())
        }
    }
}
                ...
                .await
                {
                    ...
                    Err(err) => {
                        tracing::debug!(
                            error = %err,
                            block = block_number,
                            "botchain block timestamp skipped"
                        );
                        None
                    }
                };
...
fn topic_address(address: &str) -> Result<String, String> {
    ...
    if hex.len() != 40 {
        return Err("invalid BOT Chain address".into());
    }
